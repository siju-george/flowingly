...
public class UserServiceTest : IDisposable
{
    ...
    [Theory]
    [InlineData("<expense><cost_centre>12345</cost_centre><total>100.00</total></expense>", "12345", 100.00, 20.00, 80.00)]
    [InlineData("<expense><cost_centre>67890</cost_centre><total>200.00</total></expense>", "67890", 200.00, 40.00, 160.00)]
    public void ExtractData_WithValidXml_ReturnsOkResult(string text, string costCentre, decimal total, decimal tax, decimal totalExcludingTax)
    {
        // Arrange
        var controller = new UserController();

        // Act
        var result = controller.ExtractData(text);

        // Assert
        var okResult = Assert.IsType<OkObjectResult>(result);
        //var data = Assert.IsType<object>(okResult.Value);
        var data = okResult.Value;
        Assert.Equal(costCentre, data.GetType().GetProperty("CostCentre").GetValue(data));
        Assert.Equal(total, data.GetType().GetProperty("Total").GetValue(data));
        Assert.Equal(tax, data.GetType().GetProperty("Tax").GetValue(data));
        Assert.Equal(totalExcludingTax, data.GetType().GetProperty("TotalExcludingTax").GetValue(data));
    }

    [Theory]
    [InlineData("<expense><cost_centre>67890</cost_centre><total>200.00</total>/expense>")] // xml tag not well formed. Missing closing< tag from 'expense'
    public void ExtractData_WithValidXml_ReturnsFailResult(string text)
    {
        // Arrange
        var controller = new UserController();
    ...
    //[InlineData("")]
    //[InlineData(null)]
    //public void ExtractData_WithInvalidXml_ReturnsBadRequestResult(string text)
    //{
    //    // Arrange
    ...
    //[Theory]
    //[InlineData("<expense><cost_centre>12345</cost_centre></expense>")]
    //[InlineData("<expense><total>100.00</total></expense>")]
    //public void ExtractData_WithMissingNodes_ReturnsBadRequestResult(string text)
    //{
    //    // Arrange
    //    var controller = new YourController();
